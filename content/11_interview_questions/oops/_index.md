...
|---------------|---------------------------------------------|--------------------------------------------|
| When Resolved | Compile-time (static)                       | Run-time (dynamic)                         |
| Scope         | Same class (or inherited)                   | Different classes (inheritance)            |
| Signature     | Same name, different parameters/return type | Same name, same parameters/return type     |
| Purpose       | Multiple ways to do similar tasks           | Customize inherited behavior               |
| Example       | print(String s), print(int i)               | Parent: run(); Child: run() with new logic |

- **Type Casting of Object**:
  - Converting one type to another.
  - **Upcasting**: Child to Parent (implicit, safe). E.g., Animal a = new Dog(); – treats Dog as Animal.
  - **Downcasting**: Parent to Child (explicit, risky – use instanceof check). E.g., Dog d = (Dog) a; – if a is actually Dog.
  - Used in Polymorphism: Store mixed objects in a parent-type list, then downcast as needed.
  - In Java: Throws ClassCastException if wrong.

Example: In Selenium, WebElement elem = driver.findElement(...); ((JavascriptExecutor) driver).executeScript(...); – downcasting driver.

### Syntax for OOPS and Array and Literal and Non-Literal String

This seems like a mix, perhaps from interview questions. Focusing on Java:

- **OOPS Syntax**:
  - Class: `class MyClass { int x; void method() {} }`
  - Object: `MyClass obj = new MyClass();`
  - Inheritance: `class Child extends Parent {}`
  - Interface: `interface MyInterface { void method(); }` then `class Impl implements MyInterface {}`
  - Abstract: `abstract class Abs { abstract void absMethod(); }`

- **Array Syntax**:
  - Declaration: `int[] arr;` or `int arr[];`
  - Initialization: `int[] arr = new int[5];` or `int[] arr = {1,2,3};`
  - Access: `arr[0] = 10; System.out.println(arr[0]);`
  - Multi-dimensional: `int[][] matrix = new int[3][3];`

- **Literal and Non-Literal String**:
  - **String Literal**: Created in string pool (efficient, immutable). E.g., `String s1 = "hello";` – If duplicate, reuses from pool.
  - **Non-Literal String**: Created on heap using new. E.g., `String s2 = new String("hello");` – Always new object, even if duplicate.
  - Difference: s1 == s1 (true, same reference); s1 == s2 (false, different references); s1.equals(s2) (true, same value).
  - Use literals for performance; non-literals when needing separate objects.

In OOPS Context: Strings are objects in Java, so encapsulation applies (internal char array is private). Arrays can hold objects, enabling polymorphism (e.g., Animal[] animals = {new Dog(), new Cat()};).
